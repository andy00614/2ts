...
export function transformToTs(json: any): string {
  ...
  const transferNodes = ast2TransferNode(ast);
  const interfaces = transferNodes.reduce((res, curNode) => (res += transferNode2Interface(curNode) + '\n'), '');
  return interfaces
...
const BASE_TYPES = ['string', 'number', 'bigint', 'boolean', 'symbol', 'undefined', 'object', 'function'];
function getType(val: string): Typeof | string {
  return BASE_TYPES.includes(typeof val) ? typeof val : val;
}
...
// 这个是使用于对象的形式的
export function object2Ast(json: any, astChildren: AstNew[] = <AstNew[]>initalAst.children): AstNew[] {
...
export function transferNode2Interface(transferItem: TransferInterfaceItem): string {
  ...
  const setType = (type: string) => (BASE_TYPES.includes(type) ? type : firstUppercase(type));
  let interfaceString = `interface ${firstUppercase(label)} {\n`;
  nodes.forEach((node) => {
    const { type, key } = node;
    interfaceString += `\t ${key}: ${setType(type)};\n`;
  });
  interfaceString += '};\n';
